...
class Antennas(ttk.Frame):
    ...
    def createAntenna(self):
        ...
        self.external_antenna_container.grid(row=0, column=1, sticky="nwes")
    ...
    def is_antenna_change_allowed(self):
        return not (Data.getInstance().PTT or Data.getInstance().tunerState == Data.TunerState.AUTO_TUNING)
    ...
    def on_antenna_clicked(self):
        ...
        interface.set_antenna(new_antenna)
...
class App(Tk):
    ...
    def fetch_and_push_operational_interface_frequency(self):
        ...
        self.fetchAndPushCatFrequencyJob = self.after(500, self.fetch_and_push_operational_interface_frequency)
    ...
    def restore_operational_interface_when_fallback(self):
        ...
        self.operationalInterfaceRestoreJob = self.after(10000, self.restore_operational_interface_when_fallback)
    ...
    def onTunerModeClicked(self, tuner):
        if Data.getInstance().tunerState == Data.TunerState.AUTO_TUNING:
            return
        interface.set_tuner_auto(tuner.tunerType == Tuner.TunerType.MANUAL)
        # We determine the newly shown tuner based on the tuner state, K and bypass returned by the interface

    def onBypassClicked(self, tuner):
        if Data.getInstance().tunerState == Data.TunerState.AUTO_TUNING:
            return
        interface.set_tuner_bypass(0 if tuner.tunerSubType == Tuner.TunerSubType.BYPASS else 1)
        # We determine the newly shown tuner based on the tuner state, K and bypass returned by the interface
    ...
    def onTuneClicked(self):
        ...
        interface.set_tunermode(self.tunermode)
        for tuner in self.manualTuners:
