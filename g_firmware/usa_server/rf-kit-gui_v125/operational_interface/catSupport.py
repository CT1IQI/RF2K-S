...
class CATInterface:
    ...
    def __init__(self, operational_interface_control):
        ...
        self.frequencyFailCounter = 0
        self.init_rig_if_configured()

    def init_rig_if_configured(self):
    ...
    def test_rig_and_baud_rate(self, rig, baud_rate):
        ...
        frq = self.test_frequency(test_hamlib_rig)
        if was_open:
            self.open_device()
        return frq

    def test_frequency(self, test_hamlib_rig_data):
        return _get_freq_internal_with_timeout(test_hamlib_rig_data)
    ...
    def open_device(self):
        if not self.isOpen:
            self.resetCachedFrequency()
            self.isOpen = True

    def resetCachedFrequency(self):
        self.lastFrequency = None
        self.frequencyFailCounter = 0
    ...
    def get_frequency(self):
        frequency = None
        if self.isOpen:
            frequency = _get_freq_internal_with_timeout(self.rigData)
        if frequency is None:
            self.frequencyFailCounter = self.frequencyFailCounter + 1
            if self.frequencyFailCounter < 7:
...
def _get_freq_internal_with_timeout(rig_data):
    freq_test_lock.acquire()
    try:
        return _do_with_timeout(lambda: _test_frequency(rig_data))
    finally:
        freq_test_lock.release()


def init_rig(rig_model, baud_rate):
    hamlibRig = Hamlib.Rig(rig_model)
    hamlibRig.set_conf("rig_pathname",
                 "/dev/serial/by-path/platform-fd500000.pcie-pci-0000:01:00.0-usb-0:1.4:1.0-port0")
    hamlibRig.set_conf("timeout", "50")
    hamlibRig.set_conf("retry", "3")
    hamlibRig.set_conf("serial_speed", str(baud_rate))
    return hamlibRig


def _test_frequency(test_hamlib_rig_data):
    if test_hamlib_rig_data is None:
        print("WARNING: no hamlib rig")
        return None
    ...
    if test_hamlib_rig.error_status != 0:
        print("WARNING: Error while opening device: %s" % Hamlib.rigerror(test_hamlib_rig.error_status))
        return None
    frequency = test_hamlib_rig.get_freq()
    frequency_error_status = test_hamlib_rig.error_status
    if frequency_error_status != 0:
        print("WARNING: Error while getting frequency: %s" % Hamlib.rigerror(frequency_error_status))
    test_hamlib_rig.close()
    if test_hamlib_rig.error_status != 0:
        print("WARNING: Error while closing device: %s" % Hamlib.rigerror(test_hamlib_rig.error_status))
    if frequency_error_status == 0:
        return frequency
    else:
        return None
...
def _do_with_timeout(func):
    timeout_error = None
    exchange_success = multiprocessing.Value(ctypes.c_bool)
    exchange_return = multiprocessing.Value(ctypes.c_float)
    ...
    process = multiprocessing.Process(target=f, args=(exchange_success, exchange_return,))
    process.start()
    timeout_seconds = timeout_ms / 1000.0
    process.join(timeout_seconds)
    if process.exitcode is None:
        timeout_error = "process for method " + str(
            func) + "did not exit within timeout (" + str(timeout_seconds) + " seconds)"
    elif process.exitcode != 0:
        timeout_error = "process for method " + str(
            func) + " exited with status " + str(process.exitcode) + " (timeout " + str(timeout_seconds) + " seconds)"
    if timeout_error is None and exchange_success.value is True:
        return exchange_return.value
    else:
        if timeout_error is not None:
            log("WARNING, no return value because of error: " + timeout_error)
        else:
            log("WARNING, no return value from called function")
        return None
