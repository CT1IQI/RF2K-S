...
class Tuner(TunerFrame):
    ...
    def create_footer(self):
        ...
        bypass = ttk.Label(container, text="Bypass", anchor="center", pad=(0, 5), style="Big.Button.Tuner.TLabel")
        bypass.bind("<ButtonRelease-1>", self.onBypassClicked, True)
        bypass.grid(row=0, column=0, sticky="wnes")
        reset_tuner_button = ttk.Label(container, text="Reset Tuner", anchor="center", justify="center", pad=(0, 5), style="Small.Button.Tuner.TLabel")
        reset_tuner_button.bind("<ButtonRelease-1>", self.onResetTunerClicked, True)
        reset_tuner_button.grid(row=0, column=1, padx=2, sticky="wnes")
...
def on_decrement_l(factor):
    if is_manual_tuning_allowed():
        interface.decrement_L(factor)


def on_increment_l(factor):
    if is_manual_tuning_allowed():
        interface.increment_L(factor)


def on_decrement_c(factor):
    if is_manual_tuning_allowed():
        interface.decrement_C(factor)


def on_increment_c(factor):
    if is_manual_tuning_allowed():
        interface.increment_C(factor)
