...
class TunerInterfaceSettings(ttk.Frame):
    def __init__(self, container, **kwargs):
        super().__init__(container, **kwargs)
        window_elements.create_check_button_with_text(self, "TRX tune interface",
                                                      style='Small.TCheckbutton',
                                                      variable=Config.get_instance().transceiverCanSetPowerForAutotuneVar,
                                                      command=Config.get_instance().saveTxInterfaceSettings).grid(
            row=0, column=0)
...
class SettingsTab(ttk.Frame):
    ...
    def createAdditionalSettingsContainer(self):
        ...
        self.lazy_tuner_notebook = LazyNotebook(container, small=True)
        ...
        self.lazy_tuner_notebook.add(
            lambda master: TunerStorageSettings(master, InterfaceWrapper.getInstance().get_storage_bank(), pad=15),
            "Tuner Storage", lambda tab: tab.update_ext_antenna_state())
        self.lazy_tuner_notebook.add(lambda master: TunerInterfaceSettings(master, pad=15), "Tuner Interface")
