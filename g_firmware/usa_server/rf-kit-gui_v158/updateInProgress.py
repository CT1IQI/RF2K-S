    ...
    def startUpdate(self):
        ...
        if self.filepath is None:
            try:
                ...
                log("update completed successful.")
            except Exception as e:
                log("error occured during update process, try again. If this error remains, call support")
                self.onError()
                return
        else:
            try:
                InterfaceWrapper.getInstance().update(self.filepath, lambda p: self.onProgressUpdate(self.Stage.UPDATE_CONTROLLER, p))
            except Exception as e:
                self.onError()
                return
        self.destroy()
