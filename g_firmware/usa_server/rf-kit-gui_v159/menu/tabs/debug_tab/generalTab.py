...
class GeneralTab(ttk.Frame):
    ...
    def __init__(self, delegate, container):
        ...
        self.create_10_12_15_m_gap_allowed_switch()
    ...
    def create_10_12_15_m_gap_allowed_switch(self):
        switchFrame = ttk.Frame(self.container)
        switchFrame.grid(row=4, column=0, pady=5)

        window_elements.create_check_button_with_text(switchFrame, "allow power for gap in 10/12/15m band",
                                                      style='Small.TCheckbutton', variable=Data.getInstance().vars.is_10_12_15_m_gap_power_allowed_var,
                                                      command=self.on_10_12_15_m_gap_allowed_switch).grid(row=0, column=0)

    def on_10_12_15_m_gap_allowed_switch(self):
        InterfaceWrapper.getInstance().set_10_12_15_m_gap_power_allowed(Data.getInstance().vars.is_10_12_15_m_gap_power_allowed_var.get())
