...
class OperationalInterfaceControl:
    ...
    def close_current_operational_interface(self):
        ...
        if self.selectedOperationalInterface is OperationalInterface.CAT:  # CAT poller still running in fallback
            self.catInterface.stop_poller()
        elif self.selectedOperationalInterface is OperationalInterface.UDP:  # UDP server still running in fallback
            self.udpInterface.stop_server()
    ...
    def close_current_operational_interface_for_fallback(self):
        pass
        # DO NOT STOP CAT POLLER OR ELSE THERE WILL BE NOTHING POLLED TO RECOVER FROM FALLBACK
        # DO NOT STOP UDP SERVER OR ELSE THERE WILL BE NOTHING RECEIVED TO RECOVER FROM FALLBACK
    ...
    def open_cat(self, from_fallback=False):
        if not from_fallback:
            self.catInterface.reset_frequency()
            self.catInterface.start_poller()
        self.currentOperationalInterface = OperationalInterface.CAT
        if self.catInterface.get_frequency() is None:
            self.fallback()

    def update_operational_interface(self):
        self.update_operational_interface_display_name()
        InterfaceWrapper.getInstance().set_operational_interface(self.currentOperationalInterface.value)
    ...
    def try_restore(self):
        ...
        if self.selectedOperationalInterface == OperationalInterface.CAT:
            if self.catInterface.get_frequency() is not None:
                self.open_cat(True)
        self.update_operational_interface()
    ...
    def get_and_push_frequency(self):
        ...
        if frequency is not None:
            ...
            if old_freq != frequency:
                InterfaceWrapper.getInstance().set_cat_frq(frequency)
        else:
            self.fallback()
