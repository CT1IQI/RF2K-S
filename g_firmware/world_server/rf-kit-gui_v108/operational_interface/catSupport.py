...
class SupportedRig(Enum):
 ...
 #   ELECRAFT_XG3 = (SupportedManufacturer.ELECRAFT, "Elecraft XG-3 signal generator", Hamlib.RIG_MODEL_XG3)
    ...
    ELECRAFT_K4 = (SupportedManufacturer.ELECRAFT, "K4", Hamlib.RIG_MODEL_K4)
...
class CATInterface:
    ...
    def init_rig_if_configured(self):
        ...
        saved_rig_model = None if Config.get_instance().catConfig["rig"] is None else SupportedRig[
            Config.get_instance().catConfig["rig"]]
        saved_baud_rate = Config.get_instance().catConfig["baud_rate"]
        if saved_rig_model is not None and saved_baud_rate is not None:
            self.hamlibRig = self.init_rig(saved_rig_model.hamlibRigModel, saved_baud_rate)

    def init_rig(self, rig_model, baud_rate):
    ...
    def test_rig_and_baud_rate(self, rig, baud_rate):
        ...
        if was_open:
            self.close_device()
        test_hamlib_rig = self.init_rig(rig.hamlibRigModel, baud_rate)
        frq = self.test_frequency(test_hamlib_rig)
        if was_open:
            self.open_device()
        return frq

    def test_frequency(self, test_hamlib_rig):
        if test_hamlib_rig is None:
    ...
    def set_rig_and_baud_rate(self, rig, baud_rate):
        ...
        if was_open:
            self.close_device()
        self.hamlibRig = self.init_rig(rig.hamlibRigModel, baud_rate)
        if was_open:
            self.open_device()
    ...
    def open_device(self):
        if not self.isOpen:
            ...
            self.hamlibRig.open()
            if self.hamlibRig.error_status == 0:
                self.isOpen = True
    ...
    def close_device(self):
        if self.isOpen:
            self.resetCachedFrequency()
            self.hamlibRig.close()
            if self.hamlibRig.error_status == 0:
                self.isOpen = False
            else:
                print("WARNING: Error while closing device: %s" % Hamlib.rigerror(self.hamlibRig.error_status))

    def get_frequency(self):
        frequency = None
        if self.isOpen:
            frequency = self.hamlibRig.get_freq()
            if self.hamlibRig.error_status != 0:
                frequency = None
        if frequency is None:
            self.frequencyFailCounter = self.frequencyFailCounter + 1
            if self.frequencyFailCounter < 20:
    ...
    def is_configured(self):
        return self.hamlibRig is not None
