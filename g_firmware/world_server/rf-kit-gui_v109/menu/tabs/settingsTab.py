...
class TunerOnOffSettings(ttk.Frame):
    ...
    def initialize_tuner_on_off_booleans(self):
        for on_off in interface.get_tuner_on_off_per_band():
            self.tunerOnBooleans.append(BooleanVar(value=bool(on_off)))

    def refresh_tuner_on_off_booleans(self):
        for (index, on_off) in enumerate(interface.get_tuner_on_off_per_band()):
            self.tunerOnBooleans[index].set(bool(on_off))

    def set_tuner_on_off(self, band_index):
        interface.set_tuner_on_off_for_band(band_index, int(self.tunerOnBooleans[band_index].get()))
...
class TunerStorageSettings(ttk.Frame):
    ...
    def show_bank_delete_confirmation(self, index):
        ...
        if result:
            interface.delete_storage(index)

    def on_memory_bank_selected(self):
        interface.set_storage_bank(self.currentBank.get())
    ...
    def show_antenna_delete_confirmation(self, index):
        ...
        if result:
            interface.delete_tuner_antenna(index)
    ...
    def show_ext_antenna_delete_confirmation(self):
        ...
        if result:
            interface.delete_tuner_ext_antenna(index)
...
class SettingsTab(ttk.Frame):
    ...
    def createAdditionalSettingsContainer(self):
        ...
        self.lazy_tuner_notebook.add(lambda master: TunerStorageSettings(master, interface.get_storage_bank(), pad=15), "Tuner Settings Storage", lambda tab: tab.update_ext_antenna_state())
