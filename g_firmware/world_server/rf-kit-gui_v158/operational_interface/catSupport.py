...
class SupportedRig(Enum):
    ...
    YAESU_FTDX1200 = (SupportedManufacturer.YAESU, "FTDX1200", Hamlib.RIG_MODEL_FTDX1200)
...
def get_configured_manufacturer_and_rig_and_baud_rate():
    cat_config = Config.get_instance().catConfig

    configured_manufacturer_enum_name = cat_config["manufacturer"]
    configured_manufacturer = None
    if configured_manufacturer_enum_name is not None:
        try:
            configured_manufacturer = SupportedManufacturer[configured_manufacturer_enum_name]
        except KeyError:
            log('WARNING: manufacturer ' + configured_manufacturer_enum_name + ' is not supported')

    configured_rig_enum_name = cat_config["rig"]
    saved_rig_model = None
    if configured_rig_enum_name is not None and configured_manufacturer is not None:
        try:
            saved_rig_model = SupportedRig[configured_rig_enum_name]
        except KeyError:
            log('WARNING: rig ' + configured_rig_enum_name + ' is not supported')

    configured_baud_rate = cat_config["baud_rate"]

    return configured_manufacturer, saved_rig_model, configured_baud_rate
...
class CATInterface:
    ...
    def init_rig_data_if_configured(self):
        _, saved_rig_model, saved_baud_rate = get_configured_manufacturer_and_rig_and_baud_rate()
    ...
    def multiple_get_ext_func(self, ext_funcs: list):
        log("CATInterface multiple_get_ext_func ")
        results = dict()
        was_open = self.isOpen
        if was_open:
            self.stop_poller()
        if self.rigData is None:
            log("WARNING: no hamlib rig")
        else:
            tmp_hamlib_rig = init_rig(self.rigData["model"], self.rigData["baud_rate"])
            tmp_hamlib_rig.open()
            if tmp_hamlib_rig.error_status != 0:
                log("WARNING: Error while opening device: %s" % Hamlib.rigerror(tmp_hamlib_rig.error_status))
            else:
                for func in ext_funcs:
                    log("CATInterface multiple_get_ext_func: get_ext_func " + func + " start")
                    result = tmp_hamlib_rig.get_ext_func(Hamlib.rig_ext_token_lookup(tmp_hamlib_rig.rig, func))
                    log("CATInterface multiple_get_ext_func: get_ext_func " + func + " end. result: " + str(result))
                    error_status = tmp_hamlib_rig.error_status
                    if error_status != 0:
                        log("WARNING: Error while getting %s: %s" % (func, Hamlib.rigerror(error_status)))
                    else:
                        results[func] = result
                tmp_hamlib_rig.close()
                if error_status != 0:
                    log("WARNING: Error while closing device: %s" % Hamlib.rigerror(error_status))
        if was_open:
            self.start_poller()
        log("CATInterface multiple_get_ext_func results: " + str(results))
        return results

    def get_ext_func(self, ext_func):
        return self.multiple_get_ext_func([ext_func]).get(ext_func, None)

    def set_ext_func(self, ext_func, param):
        was_open = self.isOpen
        if was_open:
            self.stop_poller()
        if self.rigData is None:
            ...
            if tmp_hamlib_rig.error_status != 0:
                ...
                tmp_hamlib_rig.set_ext_func(Hamlib.rig_ext_token_lookup(tmp_hamlib_rig.rig, ext_func), param)
                error_status = tmp_hamlib_rig.error_status
                if error_status != 0:
                    log("WARNING: Error while setting %s: %s" % (ext_func, Hamlib.rigerror(error_status)))
                tmp_hamlib_rig.close()
                if error_status != 0:
                    log("WARNING: Error while closing device: %s" % Hamlib.rigerror(error_status))
                tmp_hamlib_rig.close()
