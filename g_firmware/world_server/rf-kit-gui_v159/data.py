...
class Data:
    ...
    class Vars:
        ...
        def __init__(self, generation):
            ...
            # Tuning
            self.autotune_return_loss_threshold_var = StringVar()
            self.is_10_12_15_m_gap_power_allowed_var = BooleanVar()
    ...
    def __init__(self):
        self.guiVersion = 159
        self.minimumControllerVersion = 218
        ...
        self.is_10_12_15_m_gap_power_allowed = 0
        self.voltage = 0.
        self.current = 0.
        self.temperature = 0.
        self.P_dF = 0.
        ...
        self.autotune_return_loss_threshold = 0.
    ...
    def set_fetch_end(self, end_callable):
        self.fetchEnd = end_callable
    ...
    def updateAllValues(self):
        ...
        # deprecated
        self.is_10_12_15_m_gap_power_allowed = result[1]
        ...
        was_autotune = True if self.tunerState in (
        self.TunerState.AUTO_TUNING, self.TunerState.AUTO_TUNING_FROM_AUTO) else False
        self.tunerState = self.TunerState(result[23])
        is_autotune = True if self.tunerState in (
        self.TunerState.AUTO_TUNING, self.TunerState.AUTO_TUNING_FROM_AUTO) else False
        ...
        self.autotune_return_loss_threshold = InterfaceWrapper.getInstance().get_autotune_return_loss_threshold()
        ...
        new_ptt = True if not prevPTT and self.PTT else False
        autotune_end = True if was_autotune and not is_autotune else False
        self.updateMaxValues(new_ptt or autotune_end, autotune_end and not new_ptt)
    ...
    def update_series(self, data_series, new_value):
        if len(data_series) >= self.series_max_len:
            data_series.pop(0)
        data_series.append(new_value)

    def updateMaxValues(self, reset_overall, keep_last_when_reset_overall):
        if reset_overall and keep_last_when_reset_overall:
            last_forward = self.forward_series[-1]
            last_refl = self.reflected_series[-1]
            last_swr = self.swr_series[-1]
        self.update_series(self.forward_series, self.P_F)
        self.update_series(self.reflected_series, self.P_R)
        self.update_series(self.swr_series, self.SWR)

        self.max_P_F = max(self.forward_series)
        self.max_P_R = max(self.reflected_series)
        self.max_SWR = max(self.swr_series)

        self.overall_max_P_F = (
            max(last_forward, self.P_F) if keep_last_when_reset_overall else self.P_F) if reset_overall else max(
            self.overall_max_P_F, self.P_F)
        self.overall_max_P_R = (
            max(last_refl, self.P_R) if keep_last_when_reset_overall else self.P_R) if reset_overall else max(
            self.overall_max_P_R, self.P_R)
        self.overall_max_SWR = (
            max(last_swr, self.SWR) if keep_last_when_reset_overall else self.SWR) if reset_overall else max(
            self.overall_max_SWR, self.SWR)
    ...
    def updateAllVars(self):
        ...
        self.vars.autotune_return_loss_threshold_var.set("{:.1f}".format(self.autotune_return_loss_threshold))
        self.vars.is_10_12_15_m_gap_power_allowed_var.set(self.is_10_12_15_m_gap_power_allowed)
